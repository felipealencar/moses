 ...
 * @generated
 */
public class MosesDiagramEditorUtil {
	...
	*/
	public static boolean openDiagram(Resource diagram) throws PartInitException {
		...
		page.openEditor(new URIEditorInput(diagram.getURI()), MosesDiagramEditor.ID);
		return true;
	}
	...
	*/
	...
	public static Resource openModel(Shell shell, String description, TransactionalEditingDomain editingDomain) {
		...
		try {
			...
			MosesDiagramEditorPlugin.getInstance().logError("Unable to load resource: " + uri, we); //$NON-NLS-1$
			MessageDialog.openError(shell, Messages.MosesDiagramEditorUtil_OpenModelResourceErrorDialogTitle,
					NLS.bind(Messages.MosesDiagramEditorUtil_OpenModelResourceErrorDialogMessage, fileName));
		}
		return resource;
	}
	...
	public static void runWizard(Shell shell, Wizard wizard, String settingsKey) {
		IDialogSettings pluginDialogSettings = MosesDiagramEditorPlugin.getInstance().getDialogSettings();
	...
	*/
	public static Resource createDiagram(URI diagramURI, IProgressMonitor progressMonitor) {
		...
		progressMonitor.beginTask(Messages.MosesDiagramEditorUtil_CreateDiagramProgressTask, 3);
		final Resource diagramResource = editingDomain.getResourceSet().createResource(diagramURI);
		final String diagramName = diagramURI.lastSegment();
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(editingDomain,
				Messages.MosesDiagramEditorUtil_CreateDiagramCommandLabel, Collections.EMPTY_LIST) {
			protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info)
					throws ExecutionException {
				Sdn model = createInitialModel();
				attachModelToResource(model, diagramResource);

				Diagram diagram = ViewService.createDiagram(model, SdnEditPart.MODEL_ID,
						MosesDiagramEditorPlugin.DIAGRAM_PREFERENCES_HINT);
				if (diagram != null) {
				...
				try {
					...
					diagramResource.save(moses.diagram.part.MosesDiagramEditorUtil.getSaveOptions());
				} catch (IOException e) {

					MosesDiagramEditorPlugin.getInstance().logError("Unable to store model and diagram resources", e); //$NON-NLS-1$
				}
	...
	*/
	private static Sdn createInitialModel() {
		return MosesFactory.eINSTANCE.createSdn();
	}
