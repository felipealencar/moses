...
import moses.diagram.edit.commands.MosesCreateShortcutDecorationsCommand;
...
public class ShortcutCreationWizard extends Wizard {
	...
	public boolean performFinish() {
		CreateViewRequest.ViewDescriptor viewDescriptor = new CreateViewRequest.ViewDescriptor(
				new EObjectAdapter(referencedElementSelectionPage.getModelElement()), Node.class, null,
		...
		command = command.compose(new MosesCreateShortcutDecorationsCommand(editingDomain,
				referencedElementSelectionPage.getView(), viewDescriptor));
		try {
			OperationHistoryFactory.getOperationHistory().execute(command, new NullProgressMonitor(), null);
		} catch (ExecutionException ee) {
			MosesDiagramEditorPlugin.getInstance().logError("Unable to create shortcut", ee); //$NON-NLS-1$
		}
		return true;
	}
	...
	private static class ReferencedElementSelectionPage extends ModelElementSelectionPage {
		...
		protected boolean validatePage() {
			...
			boolean result = ViewService.getInstance().provides(Node.class, new EObjectAdapter(selectedModelElement),
					view, null, ViewUtil.APPEND, true, MosesDiagramEditorPlugin.DIAGRAM_PREFERENCES_HINT);
