 ...
 * @generated
 */
public class GroupItemSemanticEditPolicy extends MosesBaseItemSemanticEditPolicy {
	...
	*/
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (MosesVisualIDRegistry.getVisualID(incomingLink) == RuleTargetGroupRuleEditPart.VISUAL_ID) {
	...
	*/
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (MosesVisualIDRegistry.getVisualID(node)) {
			case GroupGroupHostsGroupCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (MosesVisualIDRegistry.getVisualID(cnode)) {
					case Host2EditPart.VISUAL_ID:
						for (Iterator<?> it = cnode.getTargetEdges().iterator(); it.hasNext();) {
							Edge incomingLink = (Edge) it.next();
							if (MosesVisualIDRegistry
									.getVisualID(incomingLink) == RuleSourceHostRuleEditPart.VISUAL_ID) {
							...
							if (MosesVisualIDRegistry
									.getVisualID(incomingLink) == RuleTargetHostRuleEditPart.VISUAL_ID) {
								DestroyReferenceRequest r = new DestroyReferenceRequest(
										incomingLink.getSource().getElement(), null,
										incomingLink.getTarget().getElement(), false);
								cmd.add(new DestroyReferenceCommand(r));
							...
							if (MosesVisualIDRegistry
									.getVisualID(incomingLink) == AppAppToNetworkNodeEditPart.VISUAL_ID) {
								DestroyReferenceRequest r = new DestroyReferenceRequest(
										incomingLink.getSource().getElement(), null,
										incomingLink.getTarget().getElement(), false);
								cmd.add(new DestroyReferenceCommand(r));
